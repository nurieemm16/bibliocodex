...
function catalogoShow() {
  fetch("http://localhost:8080/libros")
    ...
    .then((libros) => {
      ...
      catalogoLibros.innerHTML = libros
        .map(
          (libro) => `
                        <div>
                            <h3>TÍTULO:</h3>
...
//Función para Devolver un libro utilizando el método PUT
async function devolverLibro(event) {
  event.preventDefault();
  const nombre_usuario = document.getElementById(
    "nombre_usuario_devolver"
  ).value;
...
function actividadShow() {
  fetch("http://localhost:8080/actividades")
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "No funciona el get con las actividades" + response.statusText
        );
      }
      return response.json();
    })
    .then((actividades) => {
      ...
      actividadBiblio.innerHTML = actividades
        .map(
          (actividad) => `
                        <div>
                            <h3>ACTIVIDAD:</h3>
...
function buscarLibro() {
  // Obtiene el valor del input con id 'searchInput'
  const titulo = document.getElementById("searchInput").value;

  // Realiza una solicitud fetch a la URL del servidor con el título del libro como parámetro
  fetch(`http://localhost:8080/libros/search?titulo=${titulo}`)
    .then((response) => response.json()) // Convierte la respuesta a formato JSON
    .then((data) => {
      // Obtiene el elemento con id 'searchResults' donde se mostrarán los resultados
      let resultsDiv = document.getElementById("searchResults");
      // Limpia los resultados anteriores, si los hubiera
      resultsDiv.innerHTML = "";

      // Verifica si la data está vacía
      if (data.length === 0) {
        // Si no se encuentran libros, muestra un mensaje de error
        resultsDiv.innerHTML =
          "Lo lamentamos, este libro no se encuentra en nuestra biblioteca.";
      } else {
        // Si se encuentran libros, itera sobre cada libro en la data
        data.forEach((libro) => {
          // Agrega al div de resultados la información de cada libro
          resultsDiv.innerHTML += `
                        <div>
                            <h4>${libro.titulo}</h4>
                            ...
                            <p>Fecha de Publicación: ${new Date(
    ...
    .catch((error) => {
      // Si ocurre un error en la solicitud, se captura y se muestra en la consola
      console.error("Error:", error);
      // También se muestra un mensaje de error al usuario
      document.getElementById("searchResults").innerHTML =
        "Ocurrió un error al buscar el libro. Por favor, intenta de nuevo más tarde.";
    });
}
